...
func _ready():
	$Tween.interpolate_property($Highlight, "scale", $Highlight.scale, scale_target, 0.5, Tween.TRANS_LINEAR, Tween.EASE_IN)
	$Tween.interpolate_property($Highlight, "modulate:a", $Highlight.modulate.a, modulate_target, Tween.TRANS_LINEAR, Tween.EASE_IN_OUT)
	$Tween.start()
